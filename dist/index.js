...
function parseDate(str, date1904) {
  ...
  if (m)
    return new Date((date1904 ? dnthresh2 : dnthresh1) + ((parseInt(m[1], 10) * 60 + parseInt(m[2], 10)) * 60 + (m[3] ? parseInt(m[3].slice(1), 10) : 0)) * 1000 + (m[4] ? parseInt((m[4] + "000").slice(1, 4), 10) : 0));
  m = str.match(pdre2);
  if (m)
...
function fuzzytime1(M) {
  ...
  if (M[3]) {
    if (M[4])
      return new Date(Date.UTC(1899, 11, 31, +M[1] % 12 + (M[7] == "p" ? 12 : 0), +M[2], +M[4], parseFloat(M[3]) * 1000));
    else
      return new Date(Date.UTC(1899, 11, 31, M[7] == "p" ? 12 : 0, +M[1], +M[2], parseFloat(M[3]) * 1000));
  } else if (M[5])
    return new Date(Date.UTC(1899, 11, 31, +M[1] % 12 + (M[7] == "p" ? 12 : 0), +M[2], +M[5], M[6] ? parseFloat(M[6]) * 1000 : 0));
  else
...
function fuzzytime2(M) {
  ...
  if (M[3]) {
    if (M[4])
      return new Date(Date.UTC(1899, 11, 31, +M[1], +M[2], +M[4], parseFloat(M[3]) * 1000));
    else
      return new Date(Date.UTC(1899, 11, 31, 0, +M[1], +M[2], parseFloat(M[3]) * 1000));
  } else if (M[5])
    return new Date(Date.UTC(1899, 11, 31, +M[1], +M[2], +M[5], M[6] ? parseFloat(M[6]) * 1000 : 0));
  else
...
function write_ws_xml_cell(cell, ref, ws, opts, idx, wb, date1904) {
  ...
  if ((cell.v === undefined || cell.t === "z" && !(opts || {}).sheetStubs) && typeof cell.f !== "string" && typeof cell.z == "undefined")
    return "";
  var vv = "";
  ...
  return function parse_ws_xml_data(sdata, s, opts, guess, themes, styles, wb) {
    ...
    for (var marr = sdata.split(rowregex), mt = 0, marrlen = marr.length;mt != marrlen; ++mt) {
      ...
      for (ri = 0;ri != cells.length; ++ri) {
        ...
        d = x.slice(i);
        p = { t: "" };
        if ((cref = str_match_xml_ns(d, "v")) != null && cref[1] !== "")
    ...
    switch (Rn[3] = (raw_Rn3 = Rn[3]).toLowerCase()) {
      case "data":
        ...
        if (state[state.length - 1][1])
          break;
        if (Rn[1] === "/")
          parse_xlml_data(str.slice(didx, Rn.index), ss, dtag, state[state.length - 1][0] == "comment" ? comment : cell, { c, r }, styles, cstys[c], row, arrayf, opts, Workbook.WBProps.date1904);
        else {
          ss = "";
...
function write_xls_props(wb, cfb) {
  ...
  if (SEntries.length)
    CFB.utils.cfb_add(cfb, "/\x05SummaryInformation", write_PropertySetStream(SEntries, PSCLSID.SI, SummaryRE, SummaryPIDSI));
  if (DSEntries.length || CEntries2.length)
    CFB.utils.cfb_add(cfb, "/\x05DocumentSummaryInformation", write_PropertySetStream(DSEntries, PSCLSID.DSI, DocSummaryRE, DocSummaryPIDDSI, CEntries2.length ? CEntries2 : null, PSCLSID.UDI));
}
...
function numbers_add_ws(cfb, deps, wsidx) {
  ...
  numbers_iwa_doit(cfb, deps, tmaref, function(tmaroot, arch) {
    ...
    if (tma[70]) {
      ...
      (_a = hsoa[2]) == null || _a.forEach(function(item) {
...
// node_modules/node-xlsx/dist/chunk-ID6OMQGT.js
...
var parse = (mixed, options = {}) => {
  ...
  const workBook = isString(mixed) ? readFileSync(mixed, { dateNF, raw, ...otherOptions }) : readSync(mixed, { dateNF, raw, ...otherOptions });
...
var parseMetadata = (mixed, options = {}) => {
  const workBook = isString(mixed) ? readFileSync(mixed, options) : readSync(mixed, options);
  return Object.keys(workBook.Sheets).map((name) => {
    const sheet = workBook.Sheets[name];
...
var build = (worksheets, { parseOptions = {}, writeOptions = {}, sheetOptions = {}, ...otherOptions } = {}) => {
  ...
  return writeSync(workBook, { bookType, bookSST, type, ...otherWriteOptions });
};
var src_default = { parse, parseMetadata, build };

// src/utils.ts
function startPolling(request, options = {
  interval: 1000,
  retryOnError: true,
  finished: () => true
}) {
...
async function handleExcel(buffer) {
  const excelData = src_default.parse(buffer, {
    type: "buffer",
    sheets: WISE_BOOK,
    cellHTML: false
  });
  const schematicsData = excelData[0].data.map((arr) => {
